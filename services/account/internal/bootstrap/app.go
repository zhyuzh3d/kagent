...
import (
	...
	accountadapt "kagent/services/account/internal/adaptation"
	accountbiz "kagent/services/account/internal/business"
...
type App struct {
	...
	server   *http.Server
	executor *accountbiz.Handler
	adapter  *accountadapt.Handler
}
...
func New(cfg Config) (*App, error) {
	...
	client := accountdb.NewClient(registerURL, bootstrap, 8*time.Second)
	...
	executor := accountbiz.New(client, signingKey, instance, "http://"+addr)
	app := &App{
		Addr:           addr,
		ServiceID:      serviceID,
	...
	app.adapter = accountadapt.New(serviceID, instance, bootstrap, executor)
