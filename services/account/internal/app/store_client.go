...
func (c *Client) ListActiveSessions(ctx context.Context) ([]toolproto.AccountActiveSession, error) {
	...
	for _, row := range rows {
		userID := strings.TrimSpace(stringValue(row["user_id"]))
		sid := strings.TrimSpace(stringValue(row["sid"]))
		if userID == "" || sid == "" {
			continue
		}
...
func (c *Client) ListPublicKeys(ctx context.Context) ([]toolproto.AccountPublicKey, error) {
	...
	for _, row := range rows {
		kid := strings.TrimSpace(stringValue(row["kid"]))
		alg := strings.TrimSpace(stringValue(row["alg"]))
		publicKey := strings.TrimSpace(stringValue(row["public_key"]))
		if kid == "" || publicKey == "" {
			continue
		}
...
func (c *Client) execute(ctx context.Context, query string, args []any) (int64, int64, error) {
	...
	return int64Value(payload["rows_affected"]), int64Value(payload["last_insert_id"]), nil
}
...
func decodeSigningKey(row map[string]any) (SigningKey, error) {
	privateRaw := strings.TrimSpace(stringValue(row["private_key"]))
	...
	return SigningKey{
		...
		CreatedAtMS: int64Value(row["created_at_ms"]),
	}, nil
}
...
func decodeUserRecord(row map[string]any) (UserRecord, bool, error) {
	out := UserRecord{
		UserID:       strings.TrimSpace(stringValue(row["user_id"])),
		Username:     strings.TrimSpace(stringValue(row["username"])),
		PasswordHash: strings.TrimSpace(stringValue(row["password_hash"])),
	}
