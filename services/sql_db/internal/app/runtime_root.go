...
func DetectAppRoot() (string, error) {
	...
	if exePath, err := os.Executable(); err == nil {
		...
		add(filepath.Join(filepath.Dir(exeDir), "services", "sql_db"))
		add(filepath.Join(filepath.Dir(filepath.Dir(exeDir)), "services", "sql_db"))
	}
	if cwd, err := os.Getwd(); err == nil {
		add(cwd)
		add(filepath.Join(cwd, "services", "sql_db"))
	}
