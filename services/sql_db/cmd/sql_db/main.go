...
func main() {
	...
	if registerURL != "" {
		...
		if resp.StatusCode >= 300 {
			app.Errorf("register sql_db service to hub status=%d body=%s", resp.StatusCode, strings.TrimSpace(string(rawResp)))
			os.Exit(1)
		}
		...
		app.Infof("register sql_db service to hub status=%d", resp.StatusCode)
	}
