...
func NewSQLiteStore(path string, userID string, projectID string, threadID string) (*SQLiteStore, error) {
	...
	db, err := sqlitedriver.Open(cleanPath)
	if err != nil {
...
func (s *SQLiteStore) UpdateUserPasswordHash(userID string, passwordHash string) error {
	...
	_, err := s.db.Exec(`UPDATE users SET password_hash=? WHERE user_id=?`, strings.TrimSpace(passwordHash), uid)
	if err != nil {
...
func (s *SQLiteStore) LoadRecentSurfaceMessages(userID string, surfaceID string, limit int) ([]ChatMessage, error) {
	if s == nil || s.db == nil {
		return nil, nil
	}
	...
	if uid == "" || sid == "" {
		return nil, fmt.Errorf("user_id or surface_id is empty")
	}
	...
	return s.loadByQuery(`
		SELECT id, message_uid, project_id, thread_id, turn_id, seq, role, say, aside, action_json, ref_message_id, ref_action_slot, raw_data, parse_error, category, type, content, payload_schema_version, payload_json,
			...
			completion_status, interrupt, interrupt_at_ms, partial_text
		FROM messages
		WHERE user_id=? AND (action_json LIKE ? OR payload_json LIKE ? OR content LIKE ? OR say LIKE ? OR aside LIKE ?)
		ORDER BY id DESC
		LIMIT ?
	`, []any{uid, pattern, pattern, pattern, pattern, pattern, limit}, true)
}
