...
import { createSurfaceContextStore } from "../lib/surface-context.js";
import { callTool } from "../lib/tool-call.js";
import { loadSurfaceWindowState, saveSurfaceWindowState } from "../lib/surface-window-store.js";
...
export function createSurfaceBridge(options) {
  ...
  const surfaceContext = createSurfaceContextStore();
  const host = createPageSurfaceHost({
    ...
    onRuntimeEvent: (event) => {
      ...
      if (snapshot) {
        const descriptor = snapshotSurfaceDescriptor(event.runtime.surfaceID);
        onSurfaceEvent({
          type: event.type,
          surface_id: snapshot.surface_id,
          payload: {
            ...(snapshot.state || {}),
            surface_name: descriptor && typeof descriptor.name === "string" ? descriptor.name : "",
            surface_title: snapshot.registration && typeof snapshot.registration.title === "string" ? snapshot.registration.title : "",
          },
        });
      }
      ...
      updatePanelTitle();
      const reasonMap = {
        surface_register: "runtime_actions_change",
        surface_ready: "runtime_ready",
        surface_close: "runtime_closed",
        surface_closed: "runtime_closed",
      };
      emitStateChange(reasonMap[event.type] || "runtime_context_change");
    },
    onError: (error) => {
  ...
  function availableRuntimes() {
    return Array.from(registry.keys())
      .map((surfaceID) => runtimeSnapshot(surfaceID))
      .filter(Boolean);
  }

  function buildContextSnapshot(reason = "surface_context_sync") {
    return surfaceContext.sync({
      reason,
      registry: availableItems().map((item) => ({
  ...
  function buildSurfaceContextControls(turnId = 0) {
    const snapshot = surfaceContext.getSnapshot();
    if (!snapshot.context_version) return [];
    const controls = [
      {
        type: "send_control",
        control: "surface_registry_sync",
        turn_id: turnId,
        reason: snapshot.reason,
        extra: {
          context_version: snapshot.context_version,
          updated_at_ms: snapshot.updated_at_ms,
          active_surface_id: snapshot.active_surface_id,
          registry: snapshot.registry,
        },
      },
      {
        type: "send_control",
        control: "surface_active_change",
        turn_id: turnId,
        reason: snapshot.reason,
        extra: {
          context_version: snapshot.context_version,
    ...
    surfaceContext.listRuntimeRecords().forEach((runtimeContext) => {
      controls.push({
        type: "send_control",
        control: "surface_runtime_context",
        turn_id: turnId,
        reason: snapshot.reason,
        extra: {
          context_version: snapshot.context_version,
          updated_at_ms: snapshot.updated_at_ms,
          active_surface_id: snapshot.active_surface_id,
          surface_id: runtimeContext.surface_id,
          surface_type: runtimeContext.surface_type,
          surface_version: runtimeContext.surface_version,
          surface_name: typeof runtimeContext.catalog_name === "string" ? runtimeContext.catalog_name : "",
          surface_title: typeof runtimeContext.title === "string" ? runtimeContext.title : "",
          surface_register: runtimeContext.register_payload && typeof runtimeContext.register_payload === "object"
            ? runtimeContext.register_payload
            : {},
          surface_registration: runtimeContext.registration && typeof runtimeContext.registration === "object"
            ? runtimeContext.registration
            : {},
          surface_actions: Array.isArray(runtimeContext.actions) ? runtimeContext.actions : [],
  ...
  function buildStateReplayControls(turnId = 0) {
    return availableRuntimes()
      .map((runtime) => {
        const state = runtime && runtime.state && typeof runtime.state === "object" ? runtime.state : null;
        ...
        return {
          ...
          extra: {
            ...
            surface_type: runtime.surface_type,
            surface_version: runtime.surface_version,
            event_type: typeof state.event_type === "string" && state.event_type ? state.event_type : "state_change",
            ...
            status: typeof state.status === "string" ? state.status : "ready",
            state_version: Number.isFinite(state.state_version) ? state.state_version : 0,
            updated_at_ms: Number.isFinite(state.updated_at_ms) ? state.updated_at_ms : Date.now(),
          },
        };
      })
      .filter(Boolean);
  ...
  function emitStateChange(reason = "surface_context_sync") {
    const previousVersion = surfaceContext.getSnapshot().context_version;
    const snapshot = buildContextSnapshot(reason);
    onStateChange({
      ...
      contextControls: snapshot.context_version !== previousVersion ? buildSurfaceContextControls(0) : [],
    });
  }
  ...
  function renderWindow() {
    ...
    if (!opened) {
      updatePanelTitle();
      emitStateChange("surface_visibility_change");
      return;
    }
    mountPanelForMode();
    updatePanelTitle();
    syncWorkspaceState();
    emitStateChange(currentMode === "docked" ? "runtime_mode_docked" : "runtime_mode_floating");
  }
  ...
  function snapshotSurfaceDescriptor(surfaceID) {
    ...
    return {
      ...
      title: runtime && runtime.registration && runtime.registration.title ? runtime.registration.title : "",
    };
  }

  function emitLifecycleEvent(surfaceID, eventType, reason = "") {
    const surface = snapshotSurfaceDescriptor(surfaceID);
    if (!surface || !surface.surface_id) return;
    const isOpen = eventType === "surface_open";
    onSurfaceEvent({
      type: eventType,
      ...
      payload: {
        ...
        surface_title: surface.title || surface.name || surface.surface_id,
        event_type: eventType,
        business_state: reason ? { reason } : {},
        visible_text: isOpen
          ? `surface window opened: ${surface.name}`
          : `surface window closed: ${surface.name}${reason ? ` (${reason})` : ""}`,
        status: isOpen ? (surface.ready ? "ready" : "opening") : "closed",
        state_version: surface.state_version || 0,
  ...
  async function refreshRegistry() {
    ...
    updatePanelTitle();
    emitStateChange("catalog_refresh");
    return items;
  }
  ...
  async function ensureSurfaceOpen(surfaceID) {
    ...
    const wasAlreadyVisible = !!openedRuntime && currentSurfaceID === item.surface_id;
    if (openedRuntime) {
      ...
      if (!wasAlreadyVisible) {
        emitLifecycleEvent(item.surface_id, "surface_open", "reuse_runtime");
      }
    ...
    try {
      ...
      updatePanelTitle();
      emitStateChange("runtime_open");
      emitLifecycleEvent(item.surface_id, "surface_open", "open_surface");
      return runtime;
    } catch (error) {
  ...
  return {
    dispatchAction,
    buildStateReplayControls,
    buildSurfaceContextControls,
    getCachedState,
    getSurfaceContextSnapshot() {
      return surfaceContext.getSnapshot();
    },
    hasCapability,
    refreshRegistry,
    selectSurface,
