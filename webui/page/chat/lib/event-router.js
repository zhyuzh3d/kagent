export function createEventRouter(options) {
  const {
    app,
    chatStore,
    audioPlayback,
    setStatus,
    flashIndicator,
    appendDebug,
    onLLMDelta,
    onLLMFinal,
    onInterrupt,
  } = options;

  function syncCurrentTurn(msg) {
    const turnId = Number(msg && msg.turn_id);
    if (!Number.isFinite(turnId) || turnId <= 0) return;
    if (turnId > app.currentTurn) {
      app.currentTurn = turnId;
    }
  }

  function summarizeStateChange(msg) {
    const surfaceName = String(msg.surface_title || msg.surface_name || msg.surface_id || "surface").trim() || "surface";
    const eventType = String(msg.detail || msg.event_type || "state_change").trim() || "state_change";
    const visibleText = String(msg.visible_text || "").trim();
    if (eventType === "surface_open") {
      return `已打开 surface：${surfaceName}。`;
    }
    if (eventType === "surface_close" || eventType === "surface_closed") {
      return `已关闭 surface：${surfaceName}。`;
    }
    if (visibleText) {
      return `Surface state change：${surfaceName}。${visibleText}`;
    }
    return `Surface state change：${surfaceName}。`;
  }

  function isLifecycleStateChange(eventType) {
    return eventType === "surface_open" || eventType === "surface_close" || eventType === "surface_closed";
  }

  function handleEvent(msg) {
    const type = msg.type;
    if (!type) return;
    syncCurrentTurn(msg);

    if (type === "action_report") {
      const payload = msg.payload && typeof msg.payload === "object" ? msg.payload : {};
      const messageType = String(payload.type || "report").trim().toLowerCase() || "report";
      const sourceID = typeof payload.surface_id === "string" ? payload.surface_id.trim() : "";
      const sourceLabel = typeof payload.surface_title === "string" && payload.surface_title.trim()
        ? payload.surface_title.trim()
        : (typeof payload.surface_name === "string" ? payload.surface_name.trim() : "");
      chatStore.addChatMsg("observer", "", msg.turn_id || 0, false, app.sessionEpoch, {
        actionJSON: JSON.stringify(payload),
        rawData: JSON.stringify(payload),
        payloadJSON: JSON.stringify(payload),
        aside: "",
        category: "ai_action",
        messageType,
        sourceKind: sourceID ? "surface" : "system",
        sourceId: sourceID || "chat_server",
        sourceLabel: sourceLabel || (sourceID || "Chat Server"),
        createdAtMS: Number.isFinite(msg.ts_ms) ? msg.ts_ms : 0,
      });
      return;
    }

    if (type === "state_change") {
      const eventType = String(msg.detail || msg.event_type || "state_change").trim() || "state_change";
      if (isLifecycleStateChange(eventType)) {
        // lifecycle observer already has authoritative message_append from backend
        return;
      }
      const action = {
        type: "state_change",
        surface_id: msg.surface_id || "",
        surface_type: msg.surface_type || "",
        surface_version: msg.surface_version || "",
        state_version: Number.isFinite(msg.state_version) ? msg.state_version : 0,
        delta_or_state: msg.business_state && typeof msg.business_state === "object" ? msg.business_state : {},
      };
      const payload = {
        surface_id: msg.surface_id || "",
        surface_type: msg.surface_type || "",
        surface_version: msg.surface_version || "",
        surface_name: msg.surface_name || "",
        surface_title: msg.surface_title || "",
        status: msg.status || "",
        visible_text: msg.visible_text || "",
        state_version: Number.isFinite(msg.state_version) ? msg.state_version : 0,
        business_state: msg.business_state && typeof msg.business_state === "object" ? msg.business_state : {},
        event_type: eventType,
        source_kind: "surface",
        source_id: msg.surface_id || "",
        source_label: msg.surface_title || msg.surface_name || msg.surface_id || "surface",
      };
      chatStore.addChatMsg("observer", summarizeStateChange(msg), msg.turn_id || 0, false, app.sessionEpoch, {
        actionJSON: JSON.stringify(action),
        rawData: JSON.stringify(payload),
        payloadJSON: JSON.stringify(payload),
        category: "surface",
        messageType: "state_change",
        sourceKind: "surface",
        sourceId: msg.surface_id || "",
        sourceLabel: msg.surface_title || msg.surface_name || msg.surface_id || "surface",
        createdAtMS: Number.isFinite(msg.ts_ms) ? msg.ts_ms : 0,
      });
      return;
    }

    if (type === "message_append") {
      chatStore.appendStoredMessage(msg.observer_message, { sessionEpoch: app.sessionEpoch });
      return;
    }

    if (type === "status") {
      const value = msg.value || "";
      if ((value === "Thinking" || value === "Speaking") && (msg.turn_id || 0) > 0) {
        chatStore.noteReplyTurn(msg.turn_id);
      }
      setStatus(value);
      if (value === "Error") flashIndicator("error");
      else flashIndicator("receive");
      appendDebug("INFO", "status_update", msg.turn_id || 0, null, value);
      return;
    }

    if (type === "error") {
      setStatus("Error");
      flashIndicator("error");
      appendDebug("ERROR", "AppState", msg.turn_id || 0, null, `code=${msg.code || ""} message=${msg.message || ""}`);
      return;
    }

    if (type === "tts_warn") {
      flashIndicator("receive");
      const seq = msg.seq ? ` seq=${msg.seq}` : "";
      appendDebug("WARN", "TTSBackend", msg.turn_id || 0, msg.text || null, `code=${msg.code || "tts_warn"}${seq} message=${msg.message || "tts segment warning"}`);
      return;
    }

    if (type === "turn_nack") {
      if ((msg.turn_id || 0) === app.replyOnsetGuardTurn) {
        app.replyOnsetGuardUntil = 0;
        app.replyOnsetGuardTurn = 0;
        app.replyOnsetGuardLoggedTurn = 0;
      }
      appendDebug("WARN", "SessionControl", msg.turn_id || 0, null, `turn_nack received, activeReply=${app.activeTurnId}`);
      return;
    }

    if (type === "history_sync") {
      chatStore.handleHistorySync(msg.messages || [], msg.has_more);
      return;
    }

    if (msg.turn_id && (type === "asr_partial" || type === "asr_final") && msg.turn_id !== app.currentTurn) {
      appendDebug("DEBUG", "Network", msg.turn_id, null, `Stale msg dropped: type=${type} (currentInput=${app.currentTurn})`);
      return;
    }

    if (msg.turn_id && chatStore.isReplyPayloadType(type) && msg.turn_id < app.activeTurnId) {
      appendDebug("DEBUG", "Network", msg.turn_id, null, `Stale msg dropped: type=${type} (activeReply=${app.activeTurnId})`);
      return;
    }

    if (msg.turn_id && chatStore.isReplyPayloadType(type)) {
      chatStore.noteReplyTurn(msg.turn_id);
    }

    flashIndicator("receive");

    if (type === "asr_partial") {
      chatStore.updatePartialASR(msg.text || "", msg.turn_id);
      const text = (msg.text || "").trim();
      // Improved check: If audio is physically playing, or we are specifically in Thinking state,
      // allow interruption regardless of if VAD shifted the status to "Listening" already.
      const isActuallyBusy = audioPlayback.isSpeaking() || (app.backendState === "Thinking" || app.backendState === "Speaking");
      if (isActuallyBusy && text.length >= 2) {
        appendDebug("INFO", "SessionControl", msg.turn_id, text, "Commit-stop triggered by ASR partial");
        audioPlayback.stopPlayback();
        if (typeof onInterrupt === "function") {
          onInterrupt(msg.turn_id);
        }
      }
      return;
    }
    if (type === "asr_final") {
      chatStore.finalizeASR(msg.text || "", msg.turn_id);
      return;
    }
    if (type === "llm_delta") {
      if (typeof onLLMDelta === "function") {
        const handled = onLLMDelta({
          turnId: msg.turn_id,
          text: msg.text || "",
          message: msg,
        });
        if (handled && handled.handled) {
          if (typeof handled.content === "string") {
            chatStore.setAIMsgText(msg.turn_id, handled.content);
          }
          return;
        }
      }
      chatStore.appendAIDelta(msg.text || "", msg.turn_id);
      return;
    }
    if (type === "llm_final") {
      const finalText = (typeof msg.text === "string" && msg.text) ? msg.text : chatStore.getAIMsgText(msg.turn_id);
      if (typeof onLLMFinal === "function") {
        onLLMFinal({
          turnId: msg.turn_id,
          text: finalText,
          message: msg,
        });
      }
      chatStore.finalizeAI(msg.turn_id);
      return;
    }
    if (type === "tts_chunk") {
      audioPlayback.queueChunkMeta(msg);
    }
  }

  return {
    handleEvent,
  };
}
