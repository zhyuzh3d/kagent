...
function buildControlPayload(type, reason, turnId, text, extra) {
  const payload = { type };
  ...
  if (extra && typeof extra === "object") {
    ...
    if (typeof extra.surface_name === "string") payload.surface_name = extra.surface_name;
    if (typeof extra.surface_title === "string") payload.surface_title = extra.surface_title;
    ...
    if (Number.isFinite(extra.context_version)) payload.context_version = extra.context_version;
    if (typeof extra.active_surface_id === "string") payload.active_surface_id = extra.active_surface_id;
    if (Array.isArray(extra.registry)) payload.registry = extra.registry;
    if (extra.runtime_context && typeof extra.runtime_context === "object") payload.runtime_context = extra.runtime_context;
    if (extra.surface_register && typeof extra.surface_register === "object") payload.surface_register = extra.surface_register;
    if (extra.surface_registration && typeof extra.surface_registration === "object") payload.surface_registration = extra.surface_registration;
    if (Array.isArray(extra.surface_actions)) payload.surface_actions = extra.surface_actions;
    if (Array.isArray(extra.open_surfaces)) payload.open_surfaces = extra.open_surfaces;
    if (Number.isFinite(extra.updated_at_ms)) payload.updated_at_ms = extra.updated_at_ms;
  }
  return payload;
}

function flushPendingControls() {
  if (!wsReady() || pendingControls.length === 0) return;
  const queued = pendingControls;
  pendingControls = [];
...
function sendControl(type, reason, turnId, text, extra) {
  const payload = buildControlPayload(type, reason, turnId, text, extra);
  if (!wsReady()) {
